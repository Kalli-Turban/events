# events_1_7.py — V1.7 mit Header, Tipp des Tages (mit CTA-Button), Suche (min. 2 Zeichen), ❌-Clear, Pagination
# Supabase: SITE TIP (site_news_tipp) + EVENTS (events)
...
__APP_VERSION__ = "Frontend v1.7 (Header + Tipp + Search + Clear + Pagination)"
...
# ===== Tipp des Tages (Hilfsfunktionen) =====

def _public_url(res):
    """Extrahiert die Public URL aus Supabase-Storage-Resultaten."""
...
def init_tipp():
    """Lädt den aktuellsten veröffentlichten Tipp und erstellt ggf. einen CTA-Button."""
    try:
        res = (
            supabase.table("site_news_tipp")
            .select("*")
            .eq("published", True)
...
{row.get('body', '') or ''}"""
    btn_html = (
        ...
        f'💡 {row.get("cta_label", "Mehr lesen")} ↗</a>'
    ) if url else ""

    return gr.update(value=md, visible=True), gr.update(value=btn_html, visible=bool(btn_html))
...
def format_event_card(event: dict) -> str:
    ...
    if requires_registration:
        registration_block = (
            f"**✍️ Anmeldung:** [{email_contact}](mailto:{email_contact})" if email_contact else "**✍️ Anmeldung erforderlich**"
        )
...
def search_page(query: str, page: int, show_all: bool, start_date_val: str | None):
    try:
        ...
        # Suche: AND über Tokens, OR über Felder
...
with gr.Blocks(css=CUSTOM_CSS, title=f"{APP_TITLE} · {__APP_VERSION__}") as demo:
    ...
    # Tipp des Tages (Markdown + CTA-Button)
    with gr.Row():
    ...
    def do_search(q, show_all, start_date_val):
        qs = (q or "").strip()
        if qs and len(qs) < 2:
            # zu kurz -> normale Anzeige wie leere Suche
