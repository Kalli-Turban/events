...
# events_1_5_search.py
# Frontend mit serverseitiger Suche (ilike über mehrere Felder) + Pagination
# Basierend auf events_1_4.py
...
__APP_VERSION__ = "Frontend v1.5 (Search + Pagination)"
...
@media print {
  ...
  #kalli-events { position: absolute !important; left: 0; top: 0; width: 100%; padding: 0 !important; background: #fff !important; width: 100% !important; }
  .kalli-header, .gr-button { display: none !important; }
}
"""

# ===== Tipp-Bereich aus v1.4 =====
def _public_url(res):
...
with gr.Blocks(css=CUSTOM_CSS, title=f"{APP_TITLE} · {__APP_VERSION__}") as demo:
    ...
    # Filterleiste: Suche, Alle, Startdatum
    with gr.Row():
        suchfeld = gr.Textbox(label="🔎 Suche", placeholder="z. B. Stammtisch, Infostand …")
    ...
    q_state = gr.State("")
    current_page = gr.State(1)  # 1-basiert

    def do_search(q, show_all, start_date_val):
        md, total, pages, page = search_page(q, 1, show_all, start_date_val)
        return md, f"**{total} Treffer** · Seite {page}/{pages}", q, 1

    def go_back(q, page, show_all, start_date_val):
        md, total, pages, new_page = search_page(q, max(1, page-1), show_all, start_date_val)
        return md, f"**{total} Treffer** · Seite {new_page}/{pages}", new_page

    def go_next(q, page, show_all, start_date_val):
        md, total, pages, new_page = search_page(q, page+1, show_all, start_date_val)
        return md, f"**{total} Treffer** · Seite {new_page}/{pages}", new_page
